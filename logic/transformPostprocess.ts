...
import { adaptBezierForMergedPath } from './bezierAdaptation';

export const postProcessTransformedStrokes = (
  strokes: Stroke[],
  options: { autoClose: boolean; autoMerge: boolean; closeThreshold: number; bezierAdaptiveJoin: boolean }
): Stroke[] => {
  let next = [...strokes];
    ...
    for (let i = 0; i < next.length; i++) {
      ...
      for (let j = i + 1; j < next.length; j++) {
        ...
        if (distance(aEnd, bStart) <= options.closeThreshold) {
          const mergeIndex = mergedStroke.points.length - 1;
          let mergedPoints = [...mergedStroke.points, ...b.points.slice(1)];

          if (options.bezierAdaptiveJoin) {
            mergedPoints = adaptBezierForMergedPath(mergedPoints, mergeIndex, !!mergedStroke.isClosed, 0.22);
          }
